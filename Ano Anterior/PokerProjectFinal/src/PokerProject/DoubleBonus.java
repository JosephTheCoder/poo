...
public class DoubleBonus implements Variants{
	...
	// Set of options for winning hands
	...
	// Points for each winning hand (multiply by the betValue)
	...
	public int[] getAdvice(Card[] userHand) {
		...
		for (mask = 0x1E, x = 0; mask <= 0xF00; mask = mask << 1, x++){
			...
			if ((ranks & mask) == mask){				
				n = 0;
				holdVec = new int[4];
				//System.out.println("mask="+Long.toBinaryString(ranks));
				for(suitNb = 0; suitNb<4; suitNb++){					
					for(mask2 = 1 << x+1, i = x+1; mask2 <= (1 << x+3) ; mask2 = mask2 << 1, i++){
						System.out.println("mask2="+Long.toBinaryString(mask2));
						if((suit[suitNb] & mask & mask2) == mask2){
	 ...
	 * @param userHand a {@link Card} array.
	 * @return int the result of the hand evaluation.
	 */
